...
class Sprites:
    def __init__(self, path, cols, rows):
        ...
        self.sprite_sheet_list = list([(index % cols * w, int(index // cols) * h, w, h) for index in range(self.total_frames)])
        self.handle = list([
            (0, 0), (-hw, 0), (-w, 0),
            (0, -hh), (-hw, -hh), (-w, -hh),
            (0, -h), (-hw, -h), (-w, -h), ])
    ...
    def animation(self, surface, frames, x, y, handle=0):
        surface.blit(self.sprite_sheet, (x + self.handle[handle][0], y + self.handle[handle][1]), self.sprite_sheet_list[frames])
